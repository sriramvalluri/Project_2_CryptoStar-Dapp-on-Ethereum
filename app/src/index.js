...
const App = {
  ...
  lookUp : async function(){ 
    ...
    let id = document.getElementById("lookupId").value;
    id = parseInt(id);
    const starName = await lookUptokenIdToStarInfo(id).call();
    const ownerName = await ownerOf(id).call();
    App.setStatus(`Name of the star with id ${id} is ${starName} owned by ${ownerName}`);
  }
